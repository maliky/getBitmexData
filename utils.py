...
# faire une fonction qui me met dans un df tous les fichies d'un dossier avec leur attributs
...
def get_recent_xls_file(rang=0, srcdir=None, dcond=None):
    return get_cond_recent_file(rang=rang, srcdir=srcdir, coln='fname', fcond=is_xls_file, dcond=dcond)
...
def get_cond_recent_file(rang=0, srcdir=None, coln='fname', fcond=lambda x: op.isfile(x), dcond=None):
    ...
    srcdir (=cwd by default)"""
    return get_cond_recent_info(rang=rang, srcdir=srcdir, coln=coln, fcond=fcond, dcond=dcond)


def get_cond_recent_info(rang=0, srcdir=None, coln='fname', fcond=lambda x: op.isfile(x), dcond=None):
    """soit un rang 0 à nb fichiers dans srcdir, renvois la date du fichier à ce rang class du plus récent au plus ancien si pat (def \.xls$) en regex est dans sont fullname (avec path)
    srcdir (=cwd by default)
...
def remove_ieme_xls_file(ieme=0, srcdir=None):
    """Supprime la ieme file, is ieme = * les supprime toutes"""
    return remove_ieme_cond_files(ieme=ieme, srcdir=srcdir, cond=('extension', ext_is_xls))
...
def remove_ieme_cond_ordered_files(ieme, srcdir=None, cond=None, order=None):
    ...
    if cond is None:
        ffnames = dff.sort_values(by=sort_by, ascending=ascending)\
                     .fullname
    else:
        ffnames = dff.where(dff.loc[:, cond[0]].apply(cond[1]))\
                     .dropna()\
...
def remove_but_last_csv(srcdir=None):
    ...
    dff = get_df_files(srcdir)
    ffnames = dff.where(dff.loc[:, 'extension'].apply(lambda ext: ext != '.csv'))\
                     .dropna()\
                     .fullname
    rfiles = []
    for ffname in ffnames:
